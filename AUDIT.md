...
| Sudo-gating posture: mutations gated, reads are admin-role only | ✅ smoke-verified; ✅ unit-tested | `registerSudoOpHTTP` (`pkg/server/operations.go`) is the single chokepoint for all 🔐 admin mutations. Guarded by `TestAdminMutationRoutesRequireSudo` (`pkg/server/admin_route_policy_test.go`), which serves the REAL `registerOperations()` routes and asserts `sudo_required` (401) on every mutation when the session has no fresh sudo grant. |
...
| No secret / key material in audit `detail` JSONB | ✅ smoke-verified | write-site invariant: every admin mutation handler constructs `detail` from redacted fields only (client_id, display_name, kid, etc — never hash, secret_enc, private_pem). Smoke step 120 calls `assertAuditDetailNoSecret` on every returned event. |
