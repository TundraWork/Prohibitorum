...
| Admin HTTP API for OIDC clients / SAML SPs / upstream IdPs / signing keys / groups / app-access | ✅ | Two tiers: `registerSudoOpHTTP` (admin auth + sudo + body-size + content-type) gates high-impact mutations (OIDC client create/update/rotate-secret/delete; signing-key generate/activate/retire; upstream-IdP create/update/rotate-secret/delete; credential force-revoke); `registerAdminBodyOpHTTP` (admin auth + body controls, no sudo) gates reversible mutations (SAML SP CRUD, group CRUD + membership, per-app access grants). Audit events for every mutation (`factor` ∈ oidc_client/saml_sp/upstream_idp/signing_key/group). Full route table in `api.md` |
...
| Sudo-gating posture: high-impact mutations gated, reads + reversible mutations are admin-role only | ✅ | `hasFreshSudo` (`handle_sudo.go`) is the single chokepoint for sudo-gated mutations, applied via `registerSudoOpHTTP` (raw HTTP) + `registerSudoOp` (typed Huma). The gate is a pure read against the session — checks the recent-auth window + `SudoUntil` without consuming anything (multi-use). It gates high-impact mutations (secrets, PKI, credentials, irreversible destructive actions) — incl. `UpdateAccount` (user→admin escalation). Reversible mutations (SAML CRUD, group membership, app-access grants) use `registerAdminBodyOpHTTP` (admin-only, no sudo). Guarded by `TestAdminMutationRoutesRequireSudo` (`admin_route_policy_test.go`), which serves the REAL `registerOperations()` routes and asserts `sudo_required` (401) on every 🔐 mutation with no fresh sudo grant |
