...
| PKCE required for public clients; confidential by default with per-client opt-out | ✅ | `/oauth/authorize` enforces `require_pkce OR client_auth_method <> 'none'`; a relaxed confidential client (`require_pkce=false`, explicit operator opt-in via CLI `--require-pkce=false` / admin API `requirePkce:false`) may authorize without a `code_challenge`; a DB CHECK (`oidc_client_public_requires_pkce_check`) blocks `false` for public clients; create-time validation refuses the same combination; mismatch still rejected |
...
| Structured audit logs via `credential_event` | ✅ | `pkg/audit.Writer` writes `register`/`use`/`fail`/`revoke` for password/totp/recovery_code + `session:sudo_granted` + `factor_locked` (emitted by `throttle.RegisterFailure` on the unlocked→locked transition). Rows are best-effort: `audit.RecordOrLog` logs a structured error (factor/event/actor, no `detail`) if the DB insert fails |
...
| Management HTTP API for OIDC clients / SAML SPs / upstream IdPs / signing keys / groups / app-access | ✅ | Application creation, assignment changes, providers, signing keys, groups, accounts, and instance settings require an admin. Exact application assignees may use that application's existing detail and policy operations. `registerSudoOpHTTP` adds sudo, body-size, and content-type checks to the supplied base gate for sensitive operations such as OIDC update/rotation/deletion; `registerAdminBodyOpHTTP` applies body controls without sudo to reversible operations such as SAML updates and app-group association changes. Audit events cover each mutation. Full route table in `api.md` |
...
| Sudo-gating posture: high-impact mutations require a recent authentication grant | ✅ | `hasFreshSudo` (`handle_sudo.go`) is the single chokepoint for sudo-gated mutations, applied via `registerSudoOpHTTP` (raw HTTP) and `registerSudoOp` (typed Huma). The gate is a pure read against the session and does not consume the grant. The route's base authorization remains separate: some application mutations accept an exact assignee, while global administration requires `admin`. Reversible application-policy mutations use `registerAdminBodyOpHTTP` with exact application authorization and no sudo; global group mutations remain admin-only. `TestAdminMutationRoutesRequireSudo` serves the registered routes and asserts `sudo_required` on every configured sudo mutation without a fresh grant. |
