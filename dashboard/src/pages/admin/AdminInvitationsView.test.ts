...
describe('AdminInvitationsView', () => {
  ...
  it('shows empty state', async () => {
    get.mockImplementation(async (p: string) => p.includes('/identity-providers') ? IDPS : [])
    const w = mountView(); await flushPromises()
  ...
  it('creates a federation-bound invitation when an IdP is chosen', async () => {
    get.mockImplementation(async (p: string) => p.includes('/identity-providers') ? [{ slug: 'okta', displayName: 'Okta', disabled: false }] : [])
  ...
  it('still loads invitations when the upstream-idps fetch fails', async () => {
    get.mockImplementation(async (p: string) => { if (p.includes('/identity-providers')) throw new Error('forbidden'); return INVITES })
    const w = mountView(); await flushPromises()
    expect(w.text()).toContain('https://x/enroll/tok1')
  ...
  it('shows the bound IdP displayName in the Method column', async () => {
    ...
    get.mockImplementation(async (p: string) => p.includes('/identity-providers') ? [{ slug: 'okta', displayName: 'Okta', disabled: false }] : bound)
    const w = mountView(); await flushPromises()
