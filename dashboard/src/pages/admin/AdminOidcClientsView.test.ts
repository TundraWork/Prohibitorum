...
describe('AdminOidcClientsView', () => {
  it('submits restricted access and resets it when reopening creation', async () => {
    ...
    expect(api.post).toHaveBeenLastCalledWith('/api/prohibitorum/oidc-applications', expect.objectContaining({ accessRestricted: true }))
    await w.find('[data-test="create"]').trigger('click')
    expect(w.find('[data-test="access-restricted"]').attributes('aria-checked')).toBe('false')
    await w.find('[data-test="create-confirm"]').trigger('click'); await flushPromises()
    expect(api.post).toHaveBeenLastCalledWith('/api/prohibitorum/oidc-applications', expect.objectContaining({ accessRestricted: false }))
    w.unmount()
  })
