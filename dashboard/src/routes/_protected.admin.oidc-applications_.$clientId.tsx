...
/**
 * One OIDC application's settings, addressed by Client ID.
 *
 * The Client ID is what the server names the application by everywhere — the
 * token endpoint, the access policy, the icon URL, the manager list — so it is
 * the route segment rather than a row id the console hands out. An application
 * registered elsewhere keeps the identifier it already has.
 *
 ...
 * reads whatever else it needs for itself.
 */
export const Route = createFileRoute(
  ...
  loader: async ({ context: { queryClient }, params: { clientId } }) => {
    ...
    await queryClient.ensureQueryData(oidcAppQueryOptions(clientId));
  },
  component: AdminOidcApplication,
});
