...
- `oidc_client` — RFC 8414 / OIDC Discovery static-registration metadata: redirect_uris, post_logout_redirect_uris, allowed_scopes, require_pkce, allowed_code_challenge_methods, subject_type, logo_uri, tos_uri, policy_uri. Plus `client_auth_method` (`client_secret` / `none`), which is local rather than a Discovery value: it marks the client confidential or public, and a confidential client may use either credential channel.
