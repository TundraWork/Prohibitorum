...
func TestDefinitionValidateConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "empty subjectClaim", config: json.RawMessage(`{"configurationMode":"discovery","tokenAuthMethod":"discovery","pkceMethod":"S256","endpoints":{"authorization":null,"token":null,"userinfo":null,"jwks":null},"issuerUrl":"https://issuer.example","clientId":"client-id","scopes":["openid"],"allowedDomains":[],"usernameClaim":"preferred_username","displayNameClaim":"name","emailClaim":"email","pictureClaim":"picture","subjectClaim":"","requireVerifiedEmail":true,"allowPrivateNetwork":false}`), wantErr: true},
	} {
