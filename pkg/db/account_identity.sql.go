...
const listAccountIdentitiesByAccountPage = `-- name: ListAccountIdentitiesByAccountPage :many
SELECT ai.id, ai.account_id, ai.upstream_idp_id, ai.upstream_iss, ai.upstream_sub, ai.upstream_email, ai.linked_at, ai.confirmed_at, ip.slug AS idp_slug, ip.display_name AS idp_display_name
FROM account_identity ai
JOIN upstream_idp ip ON ip.id = ai.upstream_idp_id
WHERE ai.account_id = $1
  AND ($2::timestamptz IS NULL OR (ai.linked_at, ai.id) < ($2, $3::bigint))
ORDER BY ai.linked_at DESC, ai.id DESC
LIMIT $4
`

type ListAccountIdentitiesByAccountPageParams struct {
	AccountID     int32              `json:"accountId"`
	AfterLinkedAt pgtype.Timestamptz `json:"afterLinkedAt"`
	AfterID       int64              `json:"afterId"`
	RowLimit      int32              `json:"rowLimit"`
}

type ListAccountIdentitiesByAccountPageRow struct {
	ID             int64              `json:"id"`
	AccountID      int32              `json:"accountId"`
	UpstreamIdpID  int64              `json:"upstreamIdpId"`
	UpstreamIss    string             `json:"upstreamIss"`
...
func (q *Queries) ListAccountIdentitiesByAccountPage(ctx context.Context, arg ListAccountIdentitiesByAccountPageParams) ([]ListAccountIdentitiesByAccountPageRow, error) {
	rows, err := q.db.Query(ctx, listAccountIdentitiesByAccountPage,
		arg.AccountID,
		arg.AfterLinkedAt,
		arg.AfterID,
		arg.RowLimit,
	...
	defer rows.Close()
	var items []ListAccountIdentitiesByAccountPageRow
	for rows.Next() {
		var i ListAccountIdentitiesByAccountPageRow
		if err := rows.Scan(
			&i.ID,
			&i.AccountID,
			&i.UpstreamIdpID,
