...
The full OP surface is mounted at the **issuer root**, NOT under `/api/prohibitorum`: `/oauth/authorize`, `/oauth/token`, `/oauth/userinfo`, `/oauth/introspect`, `/oauth/revoke`, `/oidc/logout`, `/oauth/jwks`, `/.well-known/openid-configuration`. Copy-pasteable curl examples in "OIDC OP" below.
...
`/oauth/introspect` (RFC 7662) introspects **OAuth access/refresh tokens this OP issued**, with per-client ownership (a client sees only its own tokens). It does NOT introspect raw session cookies; the `token_type_hint=session` shape below is a conceptual design sketch only **(not implemented)** — treat its concrete fields as illustrative. The supported pattern for a first-party RP: run Pattern A, hold the issued access token, and introspect THAT.

Public clients cannot introspect. A public (`none`-auth) client calling `/oauth/introspect` gets `invalid_client` (401), per RFC 7662 §2.1 (the caller is a resource server and must authenticate). Use a confidential client / resource-server credential. Revocation (RFC 7009) differs — a public client may still `/oauth/revoke` its own tokens.
...
Prohibitorum is a live SAML 2.0 IdP with a GHES-compatible profile. The SP redirects the user to `/saml/sso`; Prohibitorum authenticates (reusing an existing session if present), then auto-POSTs a signed SAML Response to the SP's ACS URL. **Scope:** SP-initiated SSO + IdP-initiated SSO (opt-in) + IdP-local SLO + metadata + the `saml-sp` CLI. Front-channel (multi-SP) SLO and assertion / NameID encryption are **not implemented**.
...
| IdP metadata | `$ISSUER/saml/metadata` (signed; `validUntil`/`cacheDuration` present) |
...
| IdP-initiated launcher (opt-in) | `$ISSUER/saml/sso/init?sp=<entity_id>&RelayState=<target>` (requires `allow_idp_initiated`) |
...
The `<EntityDescriptor>` is **signed** (with the active key) and carries `validUntil` + `cacheDuration` (`configx.SAML.MetadataValidity`), so SPs can integrity-check and cache-bound it. With no active signing key it fails open and serves an unsigned descriptor rather than erroring. The metadata also re-advertises the **HTTP-POST** SSO binding (see "SSO binding").
...
This is **IdP-local** logout: it terminates the one Prohibitorum session tied to that `saml_session`. It does NOT propagate a front-channel logout to the user's other SPs — coordinated multi-SP sign-out is **not implemented**.
...
Federate sign-in to an upstream OIDC provider (Google Workspace, Okta, Keycloak, another Prohibitorum). Prohibitorum is the RP; the upstream is the OP. All three provisioning modes (`auto_provision`, `link_only`, `invite_only` — the last via `GET /enrollments/{token}/start-federation`) work end-to-end.
...
- **Upstream sign-out propagation.** Logging out of Prohibitorum does not log the user out of the upstream OP. Back-channel logout is not implemented.
...
OP endpoints are rate-limited on **identity**, not client IP (client IP is untrustworthy behind NAT / CDN, so no IP-keyed buckets): `/authorize` per `account_id`; `/token` / `/introspect` / `/revoke` per `client_id`; `/userinfo` per `sub`. RPs are machines, so caps are higher than the human-facing auth surfaces. Volumetric / edge DoS remains the reverse-proxy / WAF's responsibility.
...
3. **SAML SLO (Pattern C)**: a signed `LogoutRequest` to `/saml/slo` revokes the bound Prohibitorum session and returns a signed `LogoutResponse`. This is **IdP-local** — it does NOT propagate a front-channel logout to the user's other SPs (coordinated multi-SP sign-out is not implemented). See Pattern C → "Single Logout (SLO)".
