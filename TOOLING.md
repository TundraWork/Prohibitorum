...
**Upstream app-policy showcase (opt-in seed).** Every harness start wires the reciprocal `downstream-policy-demo` provider, then gives instance A (`prohibitorum_upstream`) the complete `dev-app` policy showcase: Alice is its delegated manager; `demo-manual-access` gives Bob `allow` and Carol `deny`; and the eleven rule groups cover leaf and composite connection, login, and avatar facts. The opt-in seed is idempotent on reused databases and is recreated after `--fresh`; instance B and a normal `dev-seed` do not receive the showcase. To load it explicitly against the loopback development database after federation wiring, set the upstream `PROHIBITORUM_PUBLIC_ORIGIN`, `PROHIBITORUM_DATABASE_URL`, and `PROHIBITORUM_DATA_ENCRYPTION_KEY_V1`, then run `prohibitorum dev-seed --app-policy-demo`.
