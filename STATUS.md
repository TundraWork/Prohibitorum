...
- Migrations:
  ...
  - `002_oidc.sql` — `signing_key` (unified, `use sig|enc`, `not_before`); `oidc_client` (`post_logout_redirect_uris`, `allowed_code_challenge_methods`, `client_auth_method` — renamed from `token_endpoint_auth_method` in `034`, `id_token_signed_response_alg`, `subject_type`, `application_type`, `default_max_age`, `require_auth_time`, `contacts`, `logo_uri`, `tos_uri`, `policy_uri`, `disabled`); `revoked_jti`.
